...
export const BookingPage: React.FC = () => {
  ...
  const [userName, setUserName] = useState<string>(currentUser?.name || 'Anisa Rahmawati');
  ...
  const [userContact, setUserContact] = useState<string>(currentUser?.phone || '081200000001');
  const [supervisorName, setSupervisorName] = useState<string>('Pak Arif Nugroho, S.Kom');
  ...
  useEffect(() => {
    if (currentUser) {
      setUserName(currentUser.name || 'Anisa Rahmawati');
      setUserRole(currentUser.role || 'siswa');
      setUserClass(currentUser.class || 'XI SIJA 1');
      setUserContact(currentUser.phone || '081200000001');
    }
  }, [currentUser]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newBooking = createBooking({
      ...
      userName: userName || 'Anisa Rahmawati',
      userRole: userRole || 'siswa',
      userClass: userClass || 'XI SIJA 1',
      userContact: userContact || '081200000001',
      supervisorName: supervisorName || 'Pak Arif Nugroho, S.Kom',
